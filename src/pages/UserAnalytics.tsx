...
const UserAnalytics = () => {
  ...
  const { data: scanStats, isLoading: statsLoading } = useQuery({
    queryKey: ['user-scan-analytics', user?.id],
    queryFn: async () => {
      if (!user) return null;
  ...
  // Fetch user's vulnerability statistics
  const { data: vulnStats, isLoading: vulnLoading } = useQuery({
    queryKey: ['user-vulnerability-stats', user?.id],
    queryFn: async () => {
      if (!user) return null;
      ...
      const scanIds = userScans.map(s => s.scan_id);
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">Monitor your scan activity and findings</p>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Your daily scan activity</CardDescription>
          </CardHeader>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Distribution of scan profiles you used</CardDescription>
          </CardHeader>
          <CardContent className="h-[300px]">
            <ResponsiveContainer width="100%" height="100%">
